...
export const SimpleChoiceQuestion = ({ question, state, isCorrectingQuestion, disable }: QuestionProps) => {
    ...
    useEffect(() => {
        if (preAnswer && preAnswer !== '') {
            ...
            if (getIdAnswer) {
                ...
                if (getUtag instanceof HTMLElement) {
                    getUtag.innerText = String(
                        getValueAnswer !== ''
                            ? getValueAnswer
                            : GlobalVariable.getInstance().getQuestionToCorrect()[question.questionId][
                                  parseInt(getIdAnswer)
                              ],
                    );
                    getUtag.style.color = 'red';
                }

                Object.entries(GlobalVariable.getInstance().getQuestionToCorrect()[question.questionId]).map(
                    ([idAns, value]) => {
                        if (idAns !== getIdAnswer.toString()) {
                            const getElementU = document.getElementById(`id-${idAns}`);
                            if (getElementU instanceof HTMLElement) {
                                getElementU.innerText = String(value);
                                getElementU.style.color = '#3e3f45';
                            }
                        }
                    },
                );
    ...
    }, [state.answers.answers, idAnswer, idQuestion]);
            ...
            if (containsCurlyBraces) {
                const handleClickOnUTag = () => {
                    ...
                    Object.keys(GlobalVariable.getInstance().getQuestionToCorrect()).forEach((questionId) => {
                        Object.keys(GlobalVariable.getInstance().getQuestionToCorrect()[parseInt(questionId)]).forEach(
                            (questionAnsId) => {
                                const value =
                                    GlobalVariable.getInstance().getQuestionToCorrect()[parseInt(questionId)][
                                        parseInt(questionAnsId)
                                    ];
                                if (value === data.replace(/{|}/g, '')) {
                                    if (questionId === question.questionId.toString()) {
                ...
                Object.keys(GlobalVariable.getInstance().getQuestionToCorrect()).find((questionId) => {
                    ...
                    if (foundId !== undefined) {
                        findAnswerId[questionId] = foundId;
                    }
                ...
                return index === editableIndex ? (
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            const newValue: object = {
                                [question.questionId]: `${data.replace(/{|}/g, '')}:${e.target.value}`,
                            };
                    ...
                    <u id={`id-${findAnswerId[question.questionId]}`} key={index} onClick={handleClickOnUTag}>
                        {data.replace(/{|}/g, '')}
                    </u>
                );
            }
    ...
    return !isCorrectingQuestion ? (
        ...
        <>
            {(question.suggestedAnswers as Answer[]).map((answer, aindex) => {
                ...
                return <React.Fragment key={aindex}></React.Fragment>;
            })}
            <div
